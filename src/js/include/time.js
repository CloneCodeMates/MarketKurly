includeHtml().then(() => {
    ...
    document.querySelectorAll('.sale-product__button').forEach((cartBtn) => {
        cartBtn.addEventListener('click', (e) => {
            // 해당 제품 정보 가져오기
            const saleProduct = e.target.closest('.sale-product');
            const productImg = saleProduct.querySelector('.sale-product__image');
            const productName = saleProduct.querySelector('.sale-product__name');
            const salesPrice = saleProduct.querySelector('.sale-product__sale-price span:nth-child(2)');
            const dimmedPrice = saleProduct.querySelector('.sale-product__regular-price');
